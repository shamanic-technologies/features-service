...
describe("GET /features/:featureSlug/workflow-projection (3-grain ladder)", () => {
  ...
  // ── resolvedOutcomeCount (goal-resolved expected outcome count per grain) ──────────────────────
  it("resolvedOutcomeCount: meeting-booked = clicks·v2m + replies·r2m; coherent (spent/count == cost-per-outcome)", async () => {
    mockFetch({
      audiences: [{ id: "aud-1" }],
      audienceCost: [{ dimensions: { audienceId: "aud-1" }, totalCostInUsdCents: "40000", runCount: 8 }],
      audienceCouples: [{ dimensions: { audienceId: "aud-1", workflowSlug: "wf-a" }, totalCostInUsdCents: "40000", runCount: 8 }],
      audienceEngagement: { "aud-1": [emailGroup("wf-a", 2, 1, 4)] }, // $400 / 2 clicks / 1 reply / 4 contacted
    });
    const res = await request(app).get(`${URL_BASE}?brandId=b1&goal=meetingBooked`).set(AUTH);
    const aud = rowFor(res.body, "dyn-a", "aud-1").estimatesByGrain.audience;
    // v2m 0.05, r2m 0.40 → 2·0.05 + 1·0.40 = 0.5
    expect(aud.resolvedOutcomeCount).toBeCloseTo(0.5, 6);
    // spent / count == the grain's cost-per-outcome (meeting cost). 400 / 0.5 = 800.
    const meetingCost = aud.projected.costPerMeetingBookedUsd;
    expect(aud.evidence.spentUsd / aud.resolvedOutcomeCount).toBeCloseTo(meetingCost, 3);
    expect(meetingCost).toBeCloseTo(800, 3);
  });

  it("resolvedOutcomeCount: websiteVisit = clicks (the visit IS the outcome); spent/count == CPC", async () => {
    const SINGLE = { ...ECONOMICS, visitToPaidClientPct: 5, replyToPaidClientPct: 20 };
    mockFetch({ economics: SINGLE });
    const res = await request(app).get(`${URL_BASE}?brandId=b1&goal=website_visits`).set(AUTH);
    const brand = rowFor(res.body, "dyn-a").estimatesByGrain.brand; // 25 clicks, $500
    expect(brand.resolvedOutcomeCount).toBeCloseTo(25, 6);
    expect(brand.evidence.spentUsd / brand.resolvedOutcomeCount).toBeCloseTo(20, 6); // CPC $500/25
  });

  it("resolvedOutcomeCount: combined sales = max(clicks·v2pc, replies·r2pc); spent/count == cost-per-sale", async () => {
    const SINGLE = { ...ECONOMICS, visitToPaidClientPct: 5, replyToPaidClientPct: 20 };
    mockFetch({ economics: SINGLE });
    const res = await request(app).get(`${URL_BASE}?brandId=b1&goal=sales`).set(AUTH);
    const brand = rowFor(res.body, "dyn-a").estimatesByGrain.brand; // 25 clicks, 5 replies, $500
    // max(25·0.05, 5·0.20) = max(1.25, 1.0) = 1.25 (visit is the best channel here)
    expect(brand.resolvedOutcomeCount).toBeCloseTo(1.25, 6);
    // spent / count = 500 / 1.25 = 400 = cost-per-sale (MIN of the two paid-client costs).
    expect(brand.evidence.spentUsd / brand.resolvedOutcomeCount).toBeCloseTo(400, 3);
  });

  it("resolvedOutcomeCount: 0 for a floored grain (spend but 0 observed outcome — a bad/cold arm)", async () => {
    mockFetch({
      audiences: [{ id: "aud-1" }],
      audienceCost: [{ dimensions: { audienceId: "aud-1" }, totalCostInUsdCents: "1000", runCount: 8 }],
      audienceCouples: [{ dimensions: { audienceId: "aud-1", workflowSlug: "wf-a" }, totalCostInUsdCents: "1000", runCount: 8 }],
      audienceEngagement: { "aud-1": [emailGroup("wf-a", 0, 0, 1)] }, // spend but 0 clicks / 0 replies
    });
    const res = await request(app).get(`${URL_BASE}?brandId=b1&goal=meetingBooked`).set(AUTH);
    const aud = rowFor(res.body, "dyn-a", "aud-1").estimatesByGrain.audience;
    expect(aud.evidence.spentUsd).toBeCloseTo(10, 6); // floored unit costs, but ...
    expect(aud.resolvedOutcomeCount).toBe(0); // ... the OUTCOME COUNT is a true 0 (observed nothing)
  });

  it("resolvedOutcomeCount: absent audience-level evidence → no audience grain at all (cold arm)", async () => {
    // aud-1 is active but never ran dyn-b → the (aud-1 × dyn-b) row floors to brand/crossOrg with NO
    // audience grain, so there is no audience-grain resolvedOutcomeCount to read = a cold arm.
    mockFetch({
      audiences: [{ id: "aud-1" }],
      audienceCost: [{ dimensions: { audienceId: "aud-1" }, totalCostInUsdCents: "40000", runCount: 8 }],
      audienceCouples: [{ dimensions: { audienceId: "aud-1", workflowSlug: "wf-a" }, totalCostInUsdCents: "40000", runCount: 8 }],
    ...
    const res = await request(app).get(`${URL_BASE}?brandId=b1&goal=meetingBooked`).set(AUTH);
    const audB = rowFor(res.body, "dyn-b", "aud-1");
    expect(audB).toBeTruthy();
    expect(audB.estimatesByGrain.audience).toBeUndefined(); // no audience-level evidence for dyn-b
  });
