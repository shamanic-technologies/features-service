...
const MOCK_FEATURE = {
  ...
  inputs: [],
  outputs: [
    { key: "recipientsRepliesPositive", displayOrder: 0, defaultSort: true, sortDirection: "desc" as const },
    { key: "recipientsSent", displayOrder: 1 },
    { key: "recipientPositiveReplyRate", displayOrder: 2 },
  ],
  charts: [],
  entities: [],
  forkedFrom: null,
...
describe("GET /public/stats/ranked", () => {
  ...
  it("returns workflows ranked by objective value descending, with full stats", async () => {
    ...
    const res = await request(app)
      .get("/public/stats/ranked?featureDynastySlug=sales-cold-email&objective=recipientsRepliesPositive&groupBy=workflow&minRuns=0");
    ...
    expect(res.body.results[0].stats.recipientsSent).toBe(80);
    expect(res.body.results[0].stats.totalCostInUsdCents).toBe(2000);
    expect(res.body.results[0].stats.completedRuns).toBe(8);
    // Derived: recipientPositiveReplyRate = recipientsRepliesPositive / recipientsDelivered = 20/70
    expect(res.body.results[0].stats.recipientPositiveReplyRate).toBeCloseTo(20 / 70);
    expect(res.body.results[1].workflow.slug).toBe("sales-outreach-alpha");
    expect(res.body.results[1].stats.recipientsRepliesPositive).toBe(10);
  });
  ...
  it("uses defaultSort output when objective is not provided", async () => {
    ...
    expect(res.status).toBe(200);
    // Should default to recipientsRepliesPositive (the output with defaultSort: true)
    expect(res.body.objective).toBe("recipientsRepliesPositive");
    expect(res.body.results).toHaveLength(2);
    // Still sorted desc by recipientsRepliesPositive: Beta(20) > Alpha(10)
    expect(res.body.results[0].workflow.slug).toBe("sales-outreach-beta");
  });
  ...
  it("returns 400 when groupBy is missing", async () => {
    const res = await request(app)
      .get("/public/stats/ranked?featureDynastySlug=sales-cold-email&objective=recipientsRepliesPositive");
  ...
  it("handles zero outcomes (null stats ranked last)", async () => {
    ...
    mockFetchResponses({
      "http://email:3000/public/stats": {
        groups: [
          { key: "sales-outreach-alpha", broadcast: { recipientStats: { repliesPositive: 10, sent: 100, opened: 50 } } },
          { key: "sales-outreach-beta", broadcast: { recipientStats: { repliesPositive: 0, sent: 80, opened: 0 } } },
        ],
      },
    });
    ...
    expect(res.body.results[1].stats.recipientsRepliesPositive).toBe(0);
  });
  ...
  it("respects limit parameter (default is 3)", async () => {
    ...
    const res = await request(app)
      .get("/public/stats/ranked?featureDynastySlug=sales-cold-email&objective=recipientsRepliesPositive&groupBy=workflow&limit=1&minRuns=0");
  ...
  it("does not cap limit at 100 (regression: no hidden upper bound)", async () => {
    ...
    const res = await request(app)
      .get("/public/stats/ranked?featureDynastySlug=sales-cold-email&objective=recipientsRepliesPositive&groupBy=workflow&limit=200&minRuns=0");
  ...
  it("supports groupBy=brand with enriched brand info", async () => {
    ...
    const res = await request(app)
      .get("/public/stats/ranked?featureDynastySlug=sales-cold-email&objective=recipientsRepliesPositive&groupBy=brand&minRuns=0");
    ...
    expect(res.body.results[0].stats.recipientsSent).toBe(60);
    ...
    expect(res.body.results[1].stats.recipientsRepliesPositive).toBe(5);
  });
  ...
  it("filters out groups below minRuns threshold (default 100)", async () => {
    ...
    const res = await request(app)
      .get("/public/stats/ranked?featureDynastySlug=sales-cold-email&objective=recipientsRepliesPositive&groupBy=workflow");
  ...
  it("returns null brand name/domain when brand-service fails", async () => {
    ...
    mockFetchResponses({
      ...
      "http://email:3000/public/stats": {
        groups: [
          { key: "brand-unknown", broadcast: { recipientStats: { repliesPositive: 5, sent: 50, opened: 25 } } },
        ],
      },
    });
...
describe("GET /public/stats/best", () => {
  ...
  it("returns best workflow per count-type metric", async () => {
    ...
    expect(res.status).toBe(200);
    // recipientsRepliesPositive: Alpha 1000/10=100, Beta 2000/20=100 → tied, either wins
  ...
  it("supports groupBy=brand", async () => {
    ...
    mockFetchResponses({
      ...
      "http://email:3000/public/stats": {
        groups: [
          { key: "brand-1", broadcast: { recipientStats: { repliesPositive: 5, sent: 50, opened: 25 } } },
          { key: "brand-2", broadcast: { recipientStats: { repliesPositive: 3, sent: 60, opened: 30 } } },
        ],
      },
    });
    ...
    expect(res.body.best.recipientsRepliesPositive.brandId).toBe("brand-1");
    expect(res.body.best.recipientsRepliesPositive.value).toBe(100);
  });
});
...
describe("GET /stats/ranked (authenticated)", () => {
  ...
  it("requires auth headers", async () => {
    const res = await request(app)
      .get("/stats/ranked?featureDynastySlug=sales-cold-email&objective=recipientsRepliesPositive&groupBy=workflow");
  ...
  it("works with auth headers", async () => {
    ...
    const res = await request(app)
      .get("/stats/ranked?featureDynastySlug=sales-cold-email&objective=recipientsRepliesPositive&groupBy=workflow&minRuns=0")
