/**
 * A WORKFLOW RECOMMENDATION FOR ONE LEG, WITH NO SALES FUNNEL NAMED.
 *
 * A campaign is being redefined as (brand, offer, acquisition channel, the single LEG it is bought
 * for), because one leg belongs to several funnels at once. So a caller asking which workflow to put
 * a budget behind has a channel and a leg, and no funnel to name — and this endpoint has to answer
 ...
 * whose website funnel is worth twenty times more per client. The cheapest leg is therefore NOT the
...
describe("workflow-projection: a LEG is answerable with no sales funnel named", () => {
  beforeEach(() => {
    vi.mocked(db.query.features.findFirst).mockResolvedValue(FEATURE as any);
  });
  afterEach(() => vi.restoreAllMocks());

  it("answers a (brand, channel, leg) with a workflow recommendation and projected economics", async () => {
    mockFetch();
    ...
    // The leg is echoed with its steps BESIDE it — a consumer reads them, it never splits the key.
    expect(res.body.leg.legKey).toBe("meeting_booked_to_meeting_attended");
    expect(res.body.leg.fromStep.key).toBe("meeting_booked");
  ...
  it("a leg on SEVERAL declared funnels yields ONE answer, priced through the BEST-RETURNING one — the cheapest leg is not the best buy", async () => {
    ...
    expect(res.body.leg.candidateFunnelKeys).toEqual([
  ...
  it("STATES WHAT THE RECOMMENDATION RESTS ON — the grain, whether anything was measured, and how many outcomes", async () => {
    ...
    const cold = await get("leg=meeting_booked_to_meeting_attended");
    expect(cold.status).toBe(200);
    expect(cold.body.leg.basis).toBe("no_return_evidence");
    expect(cold.body.leg.returnPerDollar).toBeNull();
    expect(cold.body.leg.evidence.measured).toBe(false);
  ...
  it("an unrecognised leg FAILS LOUD, and a legacy-shaped spelling of a real one is accepted", async () => {
    mockFetch();
    const bad = await get("leg=signup_to_meeting_attended");
    expect(bad.status).toBe(400);
    expect(bad.body.reason).toBe("leg_unrecognised");

    const tolerated = await get("leg=Meeting-Booked-To-Meeting-Attended");
