export async function traceEvent(
  ...
  if (!url || !apiKey) {
    console.error("[features-service] RUNS_SERVICE_URL or RUNS_SERVICE_API_KEY not set, skipping trace");
    return;
  }
  try {
    ...
    console.error("[features-service] Failed to trace event:", err);
  }
}
