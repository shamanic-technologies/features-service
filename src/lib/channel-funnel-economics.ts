 ...
 *
 * A customer buys a PAIR, not a channel and not a funnel: the same chain costs a very different amount
 ...
 * (which would read as "this step is free") or a number chained through a rate nobody stated.
 *
 * ── HOW A CHAIN IS PRICED ─────────────────────────────────────────────────────────────────────────
 *
 ...
 *   2. Those unit costs are MASKED to the chain's own channel, because a chain is bought through one of
 *      them: the conversation chain prices `replyUsd / replyToMeetingPct` and the website chain
 *      `clickUsd / visitToMeetingPct` against the identical evidence. Blending them would benchmark a
 *      reply-bought meeting against clicks the chain never buys.
 *   3. The chain's own rates carry the masked unit cost down to a SALE.
 *
...
export type PairUnmeasuredReason =
  ...
  | "no_spend_recorded"
  /** Spent, but the chain's own entry step (a conversation / a website visit) was never produced. */
  | "no_entry_step_produced"
  /** No brand running this channel has declared the economics the chain is made of. */
  | "no_economics_declared";

/** Why one STEP of an otherwise-measured chain carries no price. */
export type StepUnpricedReason =
  /** The rate that carries the chain from the previous step to this one was never declared. */
  | "rate_not_declared"
  /** The declared rate is 0, so no spend reaches this step — a price would be a division by zero. */
  | "rate_is_zero";

export interface PricedStep {
  /** The step, worded exactly as brand-service words it in the chain. */
  step: string;
  /** True for the step the funnel is NAMED after — its MILESTONE. */
  milestone: boolean;
  /** What reaching this step costs, projected through the chain. Null when it cannot be priced. */
...
export interface PairEconomics {
  /** Every step of the chain, in order, each priced or explicitly unpriced. */
  steps: PricedStep[];
  ...
  evidence: {
    ...
    /** Brands whose declared economics backed the chain's rates. */
    brandCount: number;
  };
}
 ...
 * self-serve chains and the booked meeting for the two meeting chains; naming it explicitly here beats
 * inferring it from the chain's own wording downstream.
 */
...
/** A chain is bought through ONE channel; the other one's evidence is masked away so it cannot dilute
...
/** Which unit cost a chain's ENTRY step is bought with, and therefore which produced step it needs. */
...
export interface PricePairInput {
  ...
  /** Pooled all-history unit costs for the CHANNEL — before the chain's own channel mask. */
 ...
 * did the channel produce the step the chain starts from, and do we know the chain's rates. Only when
...
export function pricePair(input: PricePairInput): PairResult {
  ...
  if (funnelKey === "sales_meetings_from_conversation" || funnelKey === "sales_meetings_from_website") {
    ...
    steps = [
      // The chain's entry step IS the produced step, so its price is the channel's own unit cost.
      ...
      // the show-up rate into the booked→paid rate (`meetingChainCloseRate`), so there is no separate
