...
const fetchBrandLegEconomics = vi.fn();
vi.mock("./brand-leg-economics-client.js", () => ({
  fetchBrandLegEconomics: (...args: unknown[]) => fetchBrandLegEconomics(...args),
}));
...
const statement = (ratePct: number) => ({
  legRates: [{ fromStep: "Meeting attended", toStep: "Paid client", ratePct, stated: true }],
  offers: [],
});
...
describe("getBrandEffectiveRates — the brand's statements are read live, never off a snapshot", () => {
  ...
  it("a rate saved a moment ago is the one served, even while the measurement comes off the cache", async () => {
    ...
    expect(fetchBrandLegEconomics).toHaveBeenCalledWith("brand-1", "org-1");
  });

  it("only the lead walk goes through the snapshot layer", async () => {
    fetchBrandLegEconomics.mockResolvedValue(statement(50));
