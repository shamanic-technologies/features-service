...
const workflowProjectionResponseSchema = z.object({
  ...
  leg: z.object({
    ...
    basis: z.enum(["sole_declared_funnel", "best_returning_declared_funnel", "no_return_evidence"]).describe("WHY that funnel: it was the only declared one containing the leg; it returned best per dollar; or nothing containing the leg has a measurable return yet and the catalogue's canonical order broke the tie deterministically. Stated so a caller never assumes the answer rests on measured returns when it does not."),
...
registry.registerPath({
  ...
  responses: {
    ...
    400: { description: "Missing brandId, an unrecognised goal / funnel / pricing value, an unrecognised leg (reason='leg_unrecognised'), or a leg sent beside a funnel (reason='leg_and_funnel')", content: { "application/json": { schema: errorResponse } } },
