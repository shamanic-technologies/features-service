...
registry.registerPath({
  ...
  request: {
    ...
    query: z.object({
      ...
      workflow: z.string().optional().describe("ONE WORKFLOW DRILL-DOWN — the workflow DYNASTY slug ?groupBy=workflow emits as `workflowDynastySlug`, so the row a consumer clicked opens the workflow it named. Every block this body serves is then computed over that workflow alone: funnelSteps, roiHistory, outcomes, costEconomics, spend, the per-signal daily series and the leads. Both legs are attributed by the producer that FROZE them — the workflowSlug lead-service stored on each served lead, and the runs / email-gateway spend filtered by the dynasty (each producer resolving it to the same versioned slugs through workflow-service) — never inferred from the campaign row's CURRENT workflow, which mis-attributes every lead and dollar from before a workflow switch. COMBINES with ?campaignId= (the campaign's Workflows page drills into one of its own rows) and with ?pricing=. The unit is the DYNASTY because that is what every workflow-keyed surface in this service means by 'a workflow'; a slug workflow-service does not describe is its own dynasty of one, so every key the grouped read can emit resolves here. A workflow the scope never ran is a legitimate EMPTY answer — zero counts, zero spend, a null return — never a 404 and never a fabricated fleet estimate. Mutually exclusive with groupBy (400, reason 'workflow_and_group_by'): one drills into a workflow, the other lists many. The echo rides the response as `workflow`. Omitted → the whole scope → byte-identical to today."),
