...
const publicChannelSchema = registry.register(
  ...
  z.object({
    ...
    salesFunnels: z.array(z.object({
      ...
      minimumCommitmentDays: z.number().int().nullable().describe("The FUNNEL's own minimum commitment, in whole days. NULL = no commitment — a written statement, never a gap, and most funnels carry none today. A buyer is bound by BOTH this and the channel's `terms.minimumCommitmentDays`; the stricter of the two governs the booking."),
