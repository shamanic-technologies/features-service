...
const revenueLeadSchema = z.object({
  ...
  signup: z.boolean().describe("True when the lead reached the SIGNUP conversion — lead-service conversion tracker matched a website signup back to this lead by email (REAL producer-side attribution, the SAME matched-email join audience-stats uses). Distinct from `clicked` (the website-visit signup PROXY the funnel EV math anchors to). features-service#473."),
  signupAt: z.string().nullable().describe("ISO timestamp the signup was recorded. ALWAYS null today: lead-service exposes the matched lead (email) but NOT the conversion timestamp on any endpoint (the column exists internally — dedupe buckets by calendar-day — but is unexposed). Auto-populates when lead-service surfaces the date; no synthesis (borrowing the outreach date would be the wrong signal). features-service#473."),
  formSubmission: z.boolean().describe("True when the lead reached the FORM-SUBMISSION conversion (lead-service conversion tracker, event=form_submission — same matched-email producer attribution as `signup`). The visit-driven sibling of signup. features-service#473."),
  formSubmissionAt: z.string().nullable().describe("ISO timestamp of the form submission. ALWAYS null today (same lead-service date gap as signupAt: matched lead exposed, conversion timestamp not). Auto-populates when lead-service surfaces the date; no synthesis. features-service#473."),
...
const spendSchema = z.object({
  ...
  purchasesCount: z.number().int().optional().describe("REAL attributed purchases (lead-service conversion tracker, event=purchase, deduped, excludes 'ping') for the brand — the Purchases tile's brand-level aggregate. Equivalent to signupsCount / salesMeetingsCount / formSubmissionsCount (the purchase count previously had no aggregate here — only per-lead purchased). 0 when none; ABSENT on a cold / pre-rollout payload when lead-service didn't serve the counts; never a fabricated 0. features-service#473."),
  cppCents: z.number().nullable().optional().describe("REAL cost per purchase = totalSpentCents (COMMITTED, SAME denominator as cpsCents/totalCpcCents) / purchasesCount, USD cents. So cppCents × purchasesCount ≈ committed spend by construction. null when purchasesCount is 0 (no denominator — never a false $0). ABSENT when purchasesCount is absent. Real tracked data, not a projection. features-service#473."),
});
