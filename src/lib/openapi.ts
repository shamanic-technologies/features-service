...
/**
 * The NARROWED lead row — what `?leads=outcomes` (the DEFAULT) serves: a lead's id, the seven outcome
 * flags a browser surface reads, and the four realized-outcome timestamps. Rows that reached NOTHING
 * (every flag false, every date null) are not served at all: both browser consumers build a
 * `leadId → outcome` map out of this array and look such a row up to find it absent either way.
 *
 * Measured in prod (brand `75d7e3e8-…`, 2026-08-31): the hydrated array was 10,860,781 bytes across
 * 9,854 rows against a 43KB body; narrowed it is 19,720 bytes across 72. Ask for `?leads=full` to get
 * every row with its name, photo, org, logo, tags and firmographics — exactly one consumer does.
 */
const revenueLeadOutcomeSchema = revenueLeadSchema.pick({
  leadId: true,
  clicked: true,
...
const revenueCostEconomicsSchema = z.object({
  ...
  costOfAcquisitionPct: z.number().nullable().describe("(mature committed spend / mature pipeline) * 100 — the MATURE cohort, see maturityDays. Null when that pipeline is null or 0, or when unmeasuredReason is 'maturing'."),
  roiMultiple: z.number().nullable().describe("mature pipeline / mature committed spend — the MATURE cohort, see maturityDays. Null when that spend is 0 or that pipeline is null; with unmeasuredReason 'maturing' when the scope spent but nothing it spent is mature yet. Never 0 in place of null."),
...
// CHARGES for it; the customer performs the rest, and every time somebody moves a lead across a leg
// they are asked what that step cost them. The funnel-wide total cannot answer "what does a booked
// meeting cost me?" — it covers every leg at once — so the same statements are partitioned per rung.
...
const funnelStepSchema = z.object({
  legKey: z.string().describe("The LEG this rung IS — the single canonical identifier of the leg that moves a lead onto this step (the same value /public/channels publishes and /features/{featureSlug}/workflow-projection?leg= is asked with). Performance is measured per leg and a campaign is bought per leg, so this is what joins a rung to the campaign that bought it and to the projection that priced it. The SAME leg appears on every funnel containing it, which is why a funnel's figures are COMPOSED from its legs and why two funnels' figures must never be summed."),
...
const outcomeCausesSchema = z.object({
  priced: z.array(z.enum(["outreach", "other", "unstated"])).describe("WHOSE WINS THIS BODY PRICED — the echo of `?cause=` (default outreach), canonical order. Every outcome is counted; only these carry value into the pipeline, the return and the cost of acquisition. Always present, on every grain and every path, so a consumer can name the basis of a figure it renders beside another."),
  counts: z.object({
    outreach: z.record(z.number().int()).describe("Outcomes the customer states OUR outreach caused, per step."),
    other: z.record(z.number().int()).describe("Outcomes the customer states something else of theirs caused — a referral, a conference, their existing pipeline, another agency. REAL outcomes: they stay in lead-service's own counts, in the brand's own revenue and on the lead's own standing. Stating one honestly costs the customer nothing they can see; what it buys is leaving its value out of the return computed on OUR outreach."),
    unstated: z.record(z.number().int()).describe("Outcomes NOBODY WAS ASKED about — every statement made before the field existed, and every tracker-reported one, because a page-load tag observes a page load and cannot know why somebody bought. Never folded into either other answer: reading it as ours is the overstatement this parameter removes, and reading it as theirs would wipe out the measured pipeline of every brand on the platform overnight. Almost every outcome in the system is in this state today, which is exactly why a surface has to be able to SAY so rather than leave a reader to guess why a figure looks the way it does."),
...
const costPerOutcomeHistoryPointSchema = z.object({
  date: z.string().describe("UTC calendar day (YYYY-MM-DD) this point describes."),
  cumulativeSpendUsd: z.number().describe("COMMITTED — every dollar of committed spend (billed + open holds) from the scope's first spend up to and including this day, the SAME basis outcomes.committedSpentCents and costEconomics.committedCostUsd ride. Dated by runs-service's own cost buckets (each run's started_at); nothing is spread, smoothed or amortised."),
  cumulativeOutcomes: z.number().describe("Every outcome of this leg's step DATED up to and including this day. Fractional on a deeper leg, where the count is the observed driver signal walked forward through the funnel's own declared rates (outcomeObserved says which you are reading)."),
  costPerOutcomeUsd: z.number().nullable().describe("cumulativeSpendUsd / cumulativeOutcomes. NULL — never 0 — on any day whose cumulative outcomes OR cumulative spend is still 0: a scope that has spent with nothing to show has no price yet, and a scope with outcomes and no attributed spend did not get them for free. Null means 'we could not measure this', a 0 would mean 'this cost nothing'."),
});
...
const costPerOutcomeHistorySchema = z.object({
  ...
  daily: z.array(costPerOutcomeHistoryPointSchema).describe("One point per UTC day that has spend or a dated outcome, ascending, spanning the SAME days roiHistory spans. Days with neither are absent (never fabricated). Empty when the scope has neither spend nor a dated outcome — a scope still learning answers with an empty or all-null series rather than with nothing."),
  datedOutcomes: z.number().describe("The curve's final cumulative outcome count — the part of the scope's whole count this curve describes."),
  undatedOutcomes: z.number().describe("Outcomes in the scope's total whose signal carries NO timestamp, so they sit on no day. Reported rather than dated or dropped (the same treatment roiHistory.undatedPipelineUsd gets): datedOutcomes + undatedOutcomes is the scope's whole count, so a consumer can always tell how much of it the curve describes."),
});

const conversionRateHistoryPointSchema = z.object({
  date: z.string().describe("UTC calendar day (YYYY-MM-DD) this point describes."),
  cumulativeContacted: z.number().describe("REACH — every distinct lead this scope emailed from its first day up to and including this one, bounces and unsubscribes INCLUDED. The identical base funnelSteps.contactedRecipients states, and for the identical reason: a bounce is a real loss at the very first rung and it was paid for, so a rate that divided by the survivors would hide the people this campaign bought and never reached."),
  cumulativeOutcomes: z.number().describe("Every outcome of this leg's step DATED up to and including this day. Fractional on a deeper leg, where the count is the observed driver signal walked forward through the funnel's own declared rates (outcomeObserved says which you are reading)."),
  conversionRatePct: z.number().nullable().describe("100 x cumulativeOutcomes / cumulativeContacted. NULL ONLY when nobody has been reached yet — there is no denominator, so there is no rate. A `0` here is a MEASUREMENT: people were reached and nobody converted, which is exactly the period a customer asking 'is this working' needs to see. That null rule is deliberately the OPPOSITE of costPerOutcomeHistory.costPerOutcomeUsd, which nulls at 0 outcomes because a cost per nothing cannot be divided at all — the two must not be harmonised."),
});

const conversionRateHistorySchema = z.object({
  outcomeStep: channelStepSchema.describe("The step EVERY count here is denominated in — the scope's own leg's toStep, named by this service rather than chosen by the caller, and the byte-same step costPerOutcomeHistory.outcomeStep and learningPhase.outcomeStep report (one leader resolution serves all three, so three curves on one screen can never be measuring different steps). A consumer labels the chart with it and never picks a noun."),
  legKey: z.string().describe("The leg that step closes, canonical."),
  outcomeObserved: z.boolean().describe("TRUE ⟺ the counts are raw OBSERVATIONS (an ENTRY leg, whose driver signal IS its outcome). FALSE ⟺ each day's count is the observed driver walked forward through the funnel's declared rates, so it is fractional."),
  daily: z.array(conversionRateHistoryPointSchema).describe("One point per UTC day that reached somebody or dated an outcome, ascending. BOTH legs are CUMULATIVE since the scope's first day, for the reason roiHistory gives: outreach on a day earns conversions days or weeks later, so a period-grain rate oscillates between 0 and absurd and describes nothing. Days with neither are absent (never fabricated). Empty when the scope has reached nobody and dated no outcome."),
  datedContacted: z.number().describe("The curve's final cumulative population — the part of the scope's whole reach it describes."),
  undatedContacted: z.number().describe("Leads this scope reached whose outreach carries NO timestamp, so they sit on no day. Reported rather than dated or dropped: datedContacted + undatedContacted is the scope's whole reach."),
...
const learningPhaseSchema = z.object({
  ...
  committedSpentUsd: z.number().nullable().describe("What the LEADING campaign has committed so far on this channel set, read from the runs LEDGER on the same COMMITTED basis (actual + holds) every money figure on this body rides — so on a campaign-scoped read it IS `costEconomics.committedCostUsd`, to sub-cent rounding. Deliberately NOT the sum of the cells the price is pooled from: those are rolled up by workflow DYNASTY, so spend on a lineage since retired is absent from them, and summing them would put two numbers about one campaign's money on one body (measured in prod: $790.53 of cells against $850.35 of ledger). The cells price ONE outcome and are right to exclude a lineage that produced none; this says what has been SPENT."),
...
const featureRevenueResponseSchema = z.object({
  ...
  leads: z.union([z.array(revenueLeadOutcomeSchema), z.array(revenueLeadSchema)]).describe("The people. WHICH SHAPE depends on `?leads=`: omitted or `outcomes` (the DEFAULT) serves the NARROW row — a lead's id, its seven outcome flags and its four realized-outcome timestamps — on the rows that REACHED something; `full` serves every contacted lead fully hydrated (name, photo, org, logo, domain, tags, firmographics, every timestamp). The default moved because a body answering a question about MONEY was 99.6% people: measured in prod (brand 75d7e3e8-…, 2026-08-31) the hydrated array was 10,860,781 bytes of a 10,903,573-byte body, and the consumer's persisted cache refuses anything over 2MB, so every money card on every brand, offer, funnel and campaign page cold-skeletoned on every load. A row that reached nothing is dropped rather than narrowed: a `false` flag is this service saying 'measured, did not happen', which no consumer acts on. Whether an outcome is ATTRIBUTED AT ALL is a separate statement and is served separately — see `attributedOutcomes`, which is why a narrowed array costs nobody an answer."),
  attributedOutcomes: z.array(z.enum(["clicked", "repliedPositive", "meetingBooked", "meetingAttended", "signup", "formSubmission", "purchased"])).describe("WHICH PER-LEAD OUTCOMES THIS READ COULD ATTRIBUTE — a fact about the READ, never about the leads. A consumer that shows an outcome only once this service attributes it cannot derive that from `leads[]` any more: a brand with a live tracker and zero signups serves zero outcome-carrying rows, so the derivation would read 'signup is not attributed' and silently drop a surface from a brand measuring signups perfectly well. 'Attributed, nobody has reached it yet' and 'not measured here' are different statements and only the producer can tell them apart. Derived from the SAME evidence the funnel walk reports its rungs on, so a rung that reads null and a surface that hides can never name different producers: `clicked`/`repliedPositive` ride the fail-loud core lead read and are attributed wherever the leads were read at all; `meetingBooked`/`purchased` need EITHER the human step statements or the legacy qualifications; `meetingAttended` needs the statements alone (nothing else in the fleet observes somebody showing up); `signup`/`formSubmission` need lead-service's matched-lead email set per event. EMPTY only where the leads were never read (no funnel wired)."),
  ...
  outcomeCauses: outcomeCausesSchema,
  workflow: z.object({
    workflowDynastySlug: z.string().describe("The dynasty every figure on this body was computed over — byte the same key ?groupBy=workflow emits, so the row a consumer clicked opens the workflow it named."),
...
const featureRevenueByWorkflowResponseSchema = z.object({
  ...
  campaignIdentity: campaignIdentitySchema.optional().describe("Present ONLY on a ?campaignId= read: the campaign IDENTITY every group was totalled over — the family of rows sharing (org, brand, sales funnel, acquisition channel), its stopped ancestors included. A consumer must be able to SEE that the subject is one campaign rather than the whole brand, instead of inferring it from a number that moved. Absent on the brand-wide read."),
...
registry.registerPath({
  ...
  description:
    ...
    "With ?groupBy=workflow it is one LEAN group per WORKFLOW DYNASTY the brand has run (workflowDynastySlug + workflowDynastyName + workflowSlugs + headline.totalPipelineUsd + costEconomics + outcomes), answering which of the workflows we ran for this brand made money and which burned it — and, in `outcomes`, what that money was made of: this brand's own outreach volume, website visits, positive replies, committed spend and the cost of a visit and of a reply, per workflow. A ?campaignId= on that grouped read narrows every group to what THAT campaign (its whole identity) did and spent through each workflow, so a campaign's Workflows page states the campaign's own numbers rather than the brand's.A ?offerId= narrows it the same way one grain coarser, to the campaigns selling that offer. " +
    "?workflow=<workflowDynastySlug> drills the UN-grouped read into ONE of those workflows — the same body, narrowed on both legs by the producers that froze them — so a workflow sub-page draws the same charts a campaign Overview draws, for that workflow alone. It combines with ?campaignId= and ?pricing=; a workflow the scope never ran answers empty (zero counts, null return), never a 404.",
  tags: ["Stats"],
  request: {
    headers: identityHeaders,
    params: z.object({ featureSlug: z.string() }),
    query: z.object({
      ...
      campaignId: z.string().optional().describe("Optional campaign drill-down (ignored when groupBy=campaignId). HONOURED by groupBy=workflow: each workflow group then states what THAT campaign did and spent through that workflow, over the campaign's whole IDENTITY (the family of rows sharing org+brand+funnel+channel), so a campaign's Workflows page never renders a brand figure under a campaign's name. Combines with ?workflow= on the un-grouped read (one campaign, one workflow). Mutually exclusive with offerId — a campaign already sells exactly one offer (400)."),
      workflow: z.string().optional().describe("ONE WORKFLOW DRILL-DOWN — the workflow DYNASTY slug ?groupBy=workflow emits as `workflowDynastySlug`, so the row a consumer clicked opens the workflow it named. Every block this body serves is then computed over that workflow alone: funnelSteps, roiHistory, outcomes, costEconomics, spend, the per-signal daily series and the leads. Both legs are attributed by the producer that FROZE them — the workflowSlug lead-service stored on each served lead, and the runs / email-gateway spend filtered by the dynasty's VERSIONED slugs, which this service resolves once and passes to both (never each producer's own workflowDynastySlug lever, which they resolve by asking workflow-service and which fails for a dynasty it no longer describes -- i.e. for a RETIRED lineage, the workflow a 'which of these burned money' question is most often about) — never inferred from the campaign row's CURRENT workflow, which mis-attributes every lead and dollar from before a workflow switch. COMBINES with ?campaignId= (the campaign's Workflows page drills into one of its own rows) and with ?pricing=. The unit is the DYNASTY because that is what every workflow-keyed surface in this service means by 'a workflow'; a slug workflow-service does not describe is its own dynasty of one and is asked for under that exact slug, so every key the grouped read can emit resolves here and a retired lineage still answers with its real money (its return CURVE is the one figure that degrades to null, since runs' cost timeseries offers no slug filter). A workflow the scope never ran is a legitimate EMPTY answer — zero counts, zero spend, a null return — never a 404 and never a fabricated fleet estimate. Mutually exclusive with groupBy (400, reason 'workflow_and_group_by'): one drills into a workflow, the other lists many. The echo rides the response as `workflow`. Omitted → the whole scope → byte-identical to today."),
...
const workflowProjectionRowSchema = z.object({
  ...
  estimatesByGrain: z.object({
    ...
    campaign: grainBlockSchema.optional().describe("The same path narrowed to ONE CAMPAIGN IDENTITY (org × brand × sales funnel × acquisition channel — every member, not the newest stored row). Present ONLY when the request named a `?campaignId=`, and omitted when that campaign spent 0 on the workflow. Floored against the brand grain, so a campaign that has barely spent reads its brand's price rather than looking free."),
  ...
  rank: z.number().optional().describe("THIS WORKFLOW'S POSITION IN THE ORDER THIS SERVICE SELECTS ON — 1-based, present ⟺ the request named a `?leg=` (every funnel- and goal-keyed body is byte-unchanged). It exists so NO consumer re-derives one: a page ranking the subset it displays produces a second order that disagrees with the pick. The rank is a property of the WORKFLOW, so every row of one dynasty carries the same number and `recommendedWorkflowDynastySlug` is rank 1 BY CONSTRUCTION. A TOTAL order — no ties, no gaps — broken deterministically on the dynasty slug, with never-run workflows (the explore allowance) always after every workflow with measured evidence. A workflow whose `modelEligibility.eligible` is FALSE ranks after EVERY eligible workflow, whatever its evidence, and is never the recommendation."),
  scopeRank: z.number().optional().describe("THIS ROW'S POSITION AMONG THE ROWS IT IS COMPARABLE WITH — 1-based, present ⟺ `rank` is (the request named a `?leg=`). `rank` is scored over EVERY row a dynasty has, so the cell that won it is usually not the cell on screen: measured in prod 2026-09-13 the rank-1 workflow read $175 on its campaign row and $20.35 on the audience cell that crowned it, which makes a column ordered on `rank` look arbitrary. `scopeRank` orders the rows sharing this row's `audienceId` (null = the brand / campaign column) on their OWN resolved metric, under the same `?maximize=` objective, the same three groups and the same slug tie-break — a TOTAL order per scope, with ineligible workflows (`modelEligibility.eligible: false`) after every eligible one and never-run workflows last within each. The two ranks legitimately DISAGREE: `rank` says what we would put the customer on next, `scopeRank` says what the column they are reading says. Serve both; derive neither."),
  modelEligibility: z.object({
    modelAlias: z.string().nullable().describe("The chat-service model alias this workflow writes its content with, read from its DAG's content-generation call by workflow-service. NULL when its DAG names none (no content call, no stated model, a run-time value, or several content calls disagreeing) — never a guess."),
    modelTier: z.enum(["cheap", "strong", "frontier"]).nullable().describe("The capability tier chat-service RECORDS for that alias, read from its catalogue and NEVER derived from the alias string: `flash-pro` resolves to a Flash model and is CHEAP despite containing 'pro'. NULL = the tier is UNKNOWABLE on this request (the workflow names no model, the alias is absent from the catalogue, or a producer read failed) — see unknownTierReason."),
    eligible: z.boolean().describe("FALSE ⟺ the tier is KNOWN and this leg's rule excludes it. An unknowable tier is always TRUE: we never exclude a workflow on a gap in our own reading, and never silently."),
    ineligibleReason: z.string().nullable().describe("Why this workflow is excluded, in a form a human reads — present ⟺ eligible is false, null otherwise. Served rather than left to a consumer so two surfaces cannot word it differently."),
    unknownTierReason: z.string().nullable().describe("Why the tier could not be read, same human form — present ⟺ modelTier is null. It tells 'this workflow records no model' apart from 'chat-service was unreachable' and from 'workflow-service was unreachable', which are different problems with different owners. A row carrying this is ELIGIBLE."),
  }).optional().describe("WHETHER THE MODEL WRITING THIS WORKFLOW'S EMAILS IS RIGHT FOR THIS LEG — present ⟺ the request named a `?leg=` (every funnel- and goal-keyed body is byte-unchanged). Measured fleet-wide: the capability TIER of the model writing the content decides the outcome, and the direction depends on what the leg sells — the cheap tier badly underperforms on a leg selling a CONVERSATION, the strong and frontier tiers are wasted on one selling a WEBSITE VISIT. Every other leg restricts nothing; the study said nothing about them and nothing is invented. The row is served whatever the verdict says, with its figures and its history unchanged, but the ORDERS act on it: an excluded workflow ranks after every eligible one in `rank` and in `scopeRank`, and is never `recommendedWorkflowDynastySlug` — so campaign-service can filter on `eligible` while a customer surface can tell 'this workflow is excluded' apart from 'this workflow does not exist', including for a workflow that is excluded but has ALREADY RUN."),
...
const salesFunnelKeyEnum = z.enum(["sales_meetings_from_conversation", "sales_meetings_from_website", "website_purchases", "form_magnet", "sales_from_conversation", "sales_meetings_from_ads", "lead_forms_from_ads", "sales_from_website"]);
...
const workflowProjectionResponseSchema = z.object({
  ...
  leg: z.object({
    legKey: z.string().describe("The leg that was asked for, echoed."),
  ...
  }).optional().describe("Present ⟺ the request named a LEG (`?leg=`), and it states which of the brand's declared funnels the leg was priced through and what that answer rests on. Absent on every funnel- or goal-keyed request, so those bodies are byte-identical to what they have always been."),
  campaignIdentity: z.object({
    key: z.string(),
    funnelKey: z.string().nullable(),
    acquisitionChannel: z.string().nullable(),
    campaignIds: z.array(z.string()).describe("Every member of the identity, the stopped ancestors included."),
    liveCampaignIds: z.array(z.string()),
    representativeId: z.string().describe("The member a consumer renders the identity's line on — the live campaign when there is one."),
  }).optional().describe("Present ⟺ the request named a `?campaignId=`. WHAT THE CAMPAIGN GRAIN ANSWERED FOR — the byte-same block /revenue?campaignId= and /audience-stats carry, so the fleet speaks ONE vocabulary about one campaign. The figures are totalled over the whole IDENTITY, never over the newest stored row of a campaign that has been running for weeks."),
  observedPicks: z.object({
    last: z.object({
      campaignId: z.string().describe("The identity member the trigger ran under — a family has several and they interleave, so this says which stored row recorded it."),
      workflowSlug: z.string().describe("The VERSIONED slug runs-service froze on the trigger run. Kept so a reader can join straight back to the ledger."),
      workflowDynastySlug: z.string().describe("Its dynasty — the key `rank`, `scopeRank` and `recommendedWorkflowDynastySlug` all speak, so a pick joins to the rows beside it. A lineage the catalogue no longer describes is its OWN dynasty of one (its slug), never dropped: a retired workflow is exactly what a 'what actually ran' question is usually about."),
      workflowDynastyName: z.string().nullable().describe("Null when the catalogue describes no version of the dynasty."),
      audienceId: z.string().nullable().describe("The audience the bandit picked for this trigger. NULL is a real state — the audience write-tag is younger than the workflow one, so an older trigger states its workflow and no audience. Never substituted from a neighbouring run."),
      startedAt: z.string().describe("When the trigger ran (ISO)."),
    }).nullable().describe("The most recent pick across the WHOLE identity. Null ⟺ the campaign has never triggered — a real, empty answer, distinct from a null block."),
    recent: z.array(z.any()).describe("The most recent picks, newest first, at most the requested window — same shape as `last`. Merged across every member of the identity, so a pick made under a stopped ancestor is in the list."),
    truncated: z.boolean().describe("TRUE ⟺ the identity has more triggers than the window states, i.e. the list is a window rather than the whole history."),
  }).nullable().optional().describe("WHAT ACTUALLY RAN — the (workflow, audience) cells the bandit picked, read LIVE from the runs ledger that recorded them at write time. Present ⟺ the request named a `?campaignId=` and a non-zero `?picks=`; absent on every funnel- or goal-keyed body, so campaign-service's production workflow selection is untouched. It is emphatically NOT campaign-service's `campaigns.workflow_slug`, which is the workflow the campaign was CONFIGURED with at creation and is never rewritten when the bandit switches — badging that as 'running' states a configuration where a reader expects a fact (prod 2026-09-14: one campaign's row said `…-rudder-v3`, which had never served a lead, while `…-lithium-v6` had served 2,439). NULL is 'we could not read this' (runs unreachable), never the configured workflow and never a fabricated pick."),
  ...
  recommendationWithheldReason: z.literal("no_eligible_workflow").optional().describe("Present ⟺ a `?leg=` read has workflows but the leg's model-tier rule (`modelEligibility`) excludes EVERY one of them. `recommendedWorkflowDynastySlug` is then null by refusal, never a fall back to an excluded workflow."),
...
const audienceStatsEvidenceSchema = z.object({
  ...
  firstRunAt: z.string().datetime().nullable(),
  lastRunAt: z.string().datetime().nullable(),
  memberCount: z.number().describe("Distinct MEMBER count of the audience (people served under it — human-service membership provenance). contacted ⊆ memberCount, so memberCount − contacted is people SERVED BUT NOT YET EMAILED (a sending backlog that trends to 0 on a brand sending well) — NOT the pool left. How many people the audience can still be served is human-service's `availableToContactCount` on GET /orgs/audiences (surfaced on the customer-health board as audiences.totalRemaining). 0 when the audience has no members."),
...
const customerHealthRowSchema = z.object({
  ...
  audiences: z.object({
    count: z.number().int().describe("Number of the brand's ACTIVE audiences (human-service's list when readable, else those with evidence)."),
    totalSize: z.number().int().nullable().describe("The brand's contactable pool = Σ human-service sizeCount over its active audiences (its audiences table's Size). null when unreadable (see poolUnreadableReason), never 0."),
    totalRemaining: z.number().int().nullable().describe("How many people the brand's active audiences can still be served = Σ human-service availableToContactCount (its audiences table's Remaining; pool members outside the 3-month re-contact window). null when unreadable, never a fabricated 0 — 0 means human-service says the pool is served out."),
    pctUsed: z.number().nullable().describe("% of the pool no longer servable = (totalSize − totalRemaining) / totalSize × 100. null when unreadable or totalSize is 0."),
    servedNotContacted: z.number().int().describe("People already SERVED to the brand's audiences but not yet emailed (Σ max(memberCount − contacted, 0)) — a sending backlog, not pool left."),
    poolUnreadableReason: z.enum(["human_service_unreadable", "counts_not_stated", "not_read"]).nullable().describe("Set exactly when totalSize/totalRemaining are null: the human-service read failed, an active audience came back without its counts (a partial sum would understate), or this row's enrichment never reached the read."),
  }),
  bestAudience: z.object({
    ...
    size: z.number().int().nullable().describe("The audience's contactable pool (human-service sizeCount). null when unreadable."),
    remaining: z.number().int().nullable().describe("How many people this audience can still be served (human-service availableToContactCount). null when unreadable, never 0."),
...
const showcaseBrandFunnelsSchema = z.object({
    ...
    funnels: z.array(z.object({
      ...
      steps: z.array(z.object({
        ...
        costPerReachUsd: z.number().nullable().describe("WHAT REACHING THIS RUNG COST THE CLIENT — the funnel's COMMITTED spend over the people who reached it, in DOLLARS. OBSERVED accounting, never floored to a benchmark. Rides EVERY rung including the outreach base, on the identical formula, so a consumer renders 'cost per meeting booked' for one client and 'cost per website visit' for another with no branch. null is 'we have no figure' (nobody reached it, nothing spent, or the count is unmeasured) — NEVER 0, which would read as the outcome having been free. Dollars because the page divides nothing."),
...
const channelTermsSchema = z.object({
  ...
  minimumCommitmentDays: z.number().int().describe("How long this channel must RUN before its result is judgeable, in days — SEO states 90 because SEO takes three months to show you anything. NOT a lock-in and nothing enforces it: the only thing a buyer commits to is `dailyOperatingCostCents`. A funnel sold through this channel can state a LONGER figure of its own; `salesFunnels[].effectiveMinimumCommitmentDays` is the composed answer per pair and is the one to render."),
...
const funnelLegSchema = registry.register(
  "FunnelLeg",
  z.object({
    legKey: z.string().describe("The leg's single canonical identifier — minted and owned here, and a PUBLISHED CONTRACT: the fleet keys campaigns and budgets on it. Join it against this catalogue; never parse it back into its parts."),
...
const publicChannelSchema = registry.register(
  ...
  z.object({
    ...
    salesFunnels: z.array(z.object({
      ...
      name: z.string(),
      steps: z.array(z.string()),
      funnelMinimumCommitmentDays: z.number().int().nullable().describe("The FUNNEL's OWN minimum run length in days, BEYOND what its channel already states. NULL = this funnel adds nothing, which is every funnel today — a written statement, never a gap. Published so a reader can see WHY the effective figure is what it is; do NOT recombine it with the channel's own term, `effectiveMinimumCommitmentDays` already did that."),
      effectiveMinimumCommitmentDays: z.number().int().describe("The MINIMUM RUN LENGTH that actually answers 'how long before I can judge this', in whole days — the longer of this funnel's own figure and the channel's `terms.minimumCommitmentDays`. NEVER null: a channel always states one. It is a property of the PAIR, so the same funnel legitimately reads a different figure under a different channel. NOT a lock-in and nothing enforces it: the product has no contract period, and the only thing a buyer commits to is the channel's daily operating cost. A channel stating 90 days is saying SEO takes three months to show you anything, not that you owe us three months."),
      governedBy: z.enum(["channel", "funnel"]).describe("Which side produced `effectiveMinimumCommitmentDays`. `funnel` only when the funnel states strictly MORE than its channel; a tie means the channel already covers it. Every pair reads `channel` today."),
    })).describe("The sales funnels this channel may be SOLD THROUGH — DERIVED as every declared funnel containing one of its `stepTransitions` as a leg, so it can never drift from them. An empty list is a real statement (no deployed funnel takes any step this channel performs), not a gap."),
  }),
);

const publicSalesFunnelSchema = z.object({
  key: salesFunnelKeyEnum.describe("The wire token. NEVER renamed — billing ceilings and live brand declarations reference it — while `name` is the only one of the two a customer reads. `website_purchases` reads 'Signups' and that mismatch is deliberate, not drift."),
  name: z.string().describe("brand-service's own customer-facing name for the funnel."),
  steps: z.array(z.string()).describe("The funnel in order, in brand-service's own wording, ending at the SALE ('Paid client')."),
  entryStep: channelStepSchema.describe("The step this funnel STARTS on. Spelled in the shared step vocabulary, so it joins directly against a channel's `producibleSteps`. An ad funnel starts on the step the ad DELIVERS (a booked meeting, a lead form submitted) — the click before it is not a step anybody buys."),
  entryLegKey: z.string().describe("The entry leg's canonical identifier, so the same join can be keyed on the leg vocabulary rather than on a `start_to_<step>` string the consumer composes itself."),
});
...
registry.registerPath({
  ...
  description:
    "The published acquisition-channel catalogue: every channel a customer can book, the commercial terms they commit to before anything is measured (daily operating cost whatever the volume, minimum commitment in days, upper bound on how long until it starts producing), which LEG of a funnel it performs (the step it moves a lead FROM and the step it moves it TO), who operates it, and the sales funnels that follow from those legs (with `funnels` listing EVERY declared funnel in its own right, each stating the STEP IT STARTS ON in the same vocabulary a channel's `producibleSteps` use, so a consumer answers 'which funnels does this outcome lead into' from one read with no translation table of its own), each carrying `effectiveMinimumCommitmentDays` — the pair's MINIMUM RUN LENGTH, already composed against the channel's own term, plus `governedBy` saying which side produced it — so a payment screen renders ONE figure and never combines two of ours. That figure is a run length before the result is judgeable, NOT a lock-in: nothing enforces it and the only thing a buyer commits to is the daily operating cost. A funnel is sellable LEG BY LEG rather than only end to end, so the catalogue publishes channels for the internal steps a human performs — the ones a specialist of ours runs and the ones the customer runs themselves. An internal leg is published once per OPERATOR, and the channel's NAME says which: our AI, our agency by hand, or the customer's own team. A daily operating cost of 0 means only that no standing day-rate is charged for the channel — it does NOT imply the customer operates it, since an automated channel meters its real cost per run instead; `operatedBy` is the field that says who is on it. NO customer identity anywhere in the path — the marketing site is generated from this and must never be able to drift from what we actually charge. Every published channel is BOOKABLE: there is no availability or coming-soon flag to consult, and a channel we are slower to deliver says so through its own terms. Each offering appears EXACTLY ONCE, under the slug that is current: a feature whose slug has been RETIRED (it names its successor in `supersededBySlug` on the authenticated feature row) is not listed here and returns no pair on /public/channel-funnel-economics, while every authenticated per-brand and per-campaign read of it keeps working unchanged.",
  tags: ["Public"],
  responses: {
  ...
  z.object({
    ...
    result: z.union([
      ...
      z.object({
        ...
        reason: z.enum(["no_spend_recorded", "no_entry_step_produced", "no_economics_declared", "entry_step_not_measured"]).describe("Which INGREDIENT is missing. A pair we have not measured enough SAYS SO rather than returning a figure or an empty value a consumer would have to interpret. `entry_step_not_measured` is the ad funnels' answer and is distinct from `no_entry_step_produced`: their first step is DELIVERED by the advertising platform (a meeting booked inside the ad, a lead form filled inside it) and no signal this fleet counts observes it, so the channel may well be producing the step — we simply have no unit cost to carry down the funnel, and saying so beats pricing an ad-delivered step against click or reply evidence the funnel never buys."),
      }),
    ]),
  }),
);
...
const funnelReturnPairSchema = z.object({
  ...
  channelName: z.string(),
  funnelKey: z.enum(["sales_meetings_from_conversation", "sales_meetings_from_website", "website_purchases", "form_magnet", "sales_from_conversation", "sales_meetings_from_ads", "lead_forms_from_ads", "sales_from_website"]).describe("The sales funnel, in brand-service's own canonical vocabulary."),
  funnelName: z.string(),
