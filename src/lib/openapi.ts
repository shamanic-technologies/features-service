...
const outcomeCausesSchema = z.object({
  counted: z.array(z.enum(["outreach", "other", "unstated"])).describe("WHOSE WINS THIS BODY COUNTED — the echo of `?cause=`, in canonical order. A fact about the REQUEST, so it is ALWAYS present, on every grain and on every path: a return that silently changed basis is worse than one that did not change, because the dashboard states these figures beside each other at several grains and two of them built on different bases under the same label is one screen contradicting itself. Omitted `?cause=` echoes all three, which is what this service counted before the parameter existed."),
  counts: z.object({
...
registry.registerPath({
  ...
  request: {
    ...
    query: z.object({
      ...
      cause: z.string().optional().describe("WHOSE WINS THIS READ COUNTS — a comma-separated subset of outreach | other | unstated (any order, any case). A brand contacts people through us AND through everything else it already does (referrals, conferences, an existing pipeline, another agency), so some of the people we email buy for reasons that have nothing to do with our outreach; lead-service records the answer per statement and this parameter decides which answers the pipeline revenue, the return and the cost of acquisition are computed over. 'outreach' — the customer states OUR outreach caused it. 'other' — they state something else of theirs did: a REAL outcome that stays in their own counts, their own revenue and their own ledger, simply not one to compute OUR return on. 'unstated' — NOBODY WAS ASKED, which is every statement made before the field existed and every tracker-reported one (a page-load tag cannot know why somebody bought); it is never folded into either other answer, because reading it as ours is the overstatement this removes and reading it as theirs would wipe out the measured pipeline of every brand on the platform. A state left out drops its outcomes' RUNGS and their STATED VALUES — never both halves apart, or a lead would be priced on a deal the read is not counting — and the legacy instantly qualifications go with 'unstated', since nobody was ever asked about one. Omitted → all three, byte-identical to what this service has always counted. An unrecognised word (or a list naming no state) is a 400 with reason 'cause_unrecognised', never a silent pick. Deliberately NOT the tracker's attributed / needs_review / unmatched vocabulary, which answers whether we identified who somebody was. The echo rides every response as `outcomeCauses.counted`."),
...
const offerFunnelsResponseSchema = z.object({
  ...
  outcomeCauses: z.object({
    counted: z.array(z.enum(["outreach", "other", "unstated"])).describe("WHOSE WINS EVERY ROW COUNTED — the echo of `?cause=`, canonical order, stated ONCE for the table because a lean row carries no block of its own. Always present: the money in these rows moves with the parameter, and a consumer has to be able to name the basis of a figure it is about to render beside another. Omitted `?cause=` echoes all three. The per-state COUNTS ride the full read (`outcomeCauses.counts` on /revenue), which is where a surface explaining a figure gets them."),
  }),
