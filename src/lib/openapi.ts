...
const rankedFunnelSchema = z.object({
  funnelKey: z.string().describe("brand-service's key for the funnel — the SAME key billing funds and campaign-service paces on, so a customer can map a rank straight onto a budget."),
  ...
  unrankableReason: z.enum(["no_economics", "no_workflow_evidence", "no_paid_client_path", "no_return_defined"]).nullable().describe("Why this funnel could not be ranked. `no_paid_client_path` = the funnel has NO defined path to a paying client (a whatsapp funnel carries no whatsapp→paid rate; or its rate chain collapses to 0). `no_workflow_evidence` = no history to compare it on yet. Null ⟺ rankable."),
...
const goalArbitrationResponseSchema = z.object({
  ...
  arbitration: z.object({
    ...
    goal: goalEchoEnum.nullable().describe("The recommended funnel's goal. Null ⟺ status = unrankable."),
...
registry.registerPath({
  ...
  description:
    ...
    "A funnel with NO defined return is ranked LAST, never dropped: a whatsapp funnel has no path to a paying client (brand-service exposes no whatsapp→paid rate), and a funnel with no economics / no workflow evidence / a non-positive return is likewise listed with rank=null, rankable=false and its reason. When nothing can be ranked at all the response is arbitration.status='unrankable' with a reason — distinguishable from a recommendation, never an error that hides why. " +
    "ONE ENTRY PER FUNNEL: two funnels that optimize for the same goal (a meeting booked from a reply and from a website visit) are ranked SEPARATELY and each priced on its own declared terms, because the customer funds them separately — a merged row could not answer 'where should I move my budget?' for either. A rate a funnel does not state falls back to the brand's effective economics, never to zero. " +
    "The DECLARED SET is brand-service's to own, read from brand-service, never accepted from the caller and never inferred from the brand's single optimizationGoal (nor from the brand-wide economics row, every rate of which is server-defaulted and so signals nothing). When that declaration cannot be READ — transport, a non-OK response, or an empty list, i.e. this org has never STATED what it sells through — the endpoint FAILS LOUD (502, reason='authorized_goals_unavailable') rather than substituting a default set. " +
  ...
  responses: {
    200: { description: "Every declared funnel ranked by return per dollar + the recommended funnel's best workflow and per-audience rows (or a distinguishable unrankable verdict)", content: { "application/json": { schema: goalArbitrationResponseSchema } } },
