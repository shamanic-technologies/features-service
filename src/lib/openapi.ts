...
const pipelineActivityResponseSchema = z.object({
  ...
  summary: z.object({
    dailyBudgetUsd: z.number().nullable().describe("Sum of forecastable configured campaign daily budgets for this brand + feature. Null when no complete daily budget is configured."),
...
registry.registerPath({
  ...
  description:
    ...
    "Expected outreach/clicks use forecastable configured campaign daily budgets and workflow unit costs; expected opens use observed broadcast open rate; signups are clicks × the brand's saved visitToSignupPct / 100. Missing producer inputs return null for the affected expected values.",
  tags: ["Stats"],
  request: {
    headers: identityHeaders,
    params: z.object({ featureSlug: z.string() }),
