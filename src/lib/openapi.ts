...
const channelStepSchema = z.object({
  key: z.enum([
    ...
    "meeting_attended",
    "signup",
    // THE ONE FORM STEP, labelled "Form submitted". A form on the brand's own site and one hosted by
    // the ad platform are the same step; only the channel that performed the leg differs. The retired
    // `lead_form_submitted` spelling (and its two leg keys) is still ACCEPTED on the way in and is
    // never served.
    "form_filled",
    "paid_client",
  ]),
