...
const spendSchema = z.object({
  ...
  cpsCents: z.number().nullable().describe("Cost per signup, USD cents — PROJECTED via the shared EV funnel from the brand's ACTUAL CPC/CPPR + economics (same realized basis as ROI/CAC + workflow-projection / public cost-projection; a forecast must not inflate on reserved-but-unbilled holds). Null when no economics or no usable click/reply cost basis."),
  cpsmCents: z.number().nullable().describe("Cost per sales meeting booked, USD cents — PROJECTED via the shared EV funnel from ACTUAL spend. Null when no economics or no usable click/reply cost basis."),
});

const featureRevenueResponseSchema = z.object({
  featureSlug: z.string(),
  spend: spendSchema.nullable().describe("Canonical spend block for the Overview card — Total spent / Budget spent today / CPC each in three variants (total=committed, actual=billed, provisioned=holds; total = actual + provisioned), plus top sources + projected cost-per-signup / cost-per-sales-meeting. Present on the OVERVIEW response; null on the lensed (?lens=) response (lens pages use costPerConversionUsd); absent on grouped (?groupBy=campaignId) groups. (features-service#396, committed naming features-service#402)"),
