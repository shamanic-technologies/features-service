 ...
 *
 * DAILY BUDGET IS THE RAW CONFIGURED CEILING — the per-org usage discount is NEVER applied to it. The
 * discount is a modifier on CHARGES only (frozen gross+net per cost row in the runs/billing ledger); the
 * daily budget is a configuration value, not a charge, so it is the same number for every customer whether
 * or not they have a discount. The fleet `totalDailyBudgetUsd`/MRR/ARR are pure budget projections (budget
 * × 30 / × 365), so they are undiscounted too. (Actual-charge / realized-revenue figures — e.g. the
 * `/internal/stats/revenue` realized-spend buckets — legitimately stay net; those are not computed here.)
 *
...
export interface AccountRow {
  ...
  /**
   * The RAW configured brand daily budget in USD (billing daily-budget). The per-org usage discount is a
   * modifier on CHARGES, not on this configuration ceiling, so it is NEVER applied here — two orgs with the
   * same configured budget show the same number regardless of their discounts. Null when unset/paused.
   */
...
export interface AccountsStats {
  /** Σ RAW configured daily budget over ACTIVE rows only (USD; undiscounted — a budget is not a charge). The staff-page figure. */
  totalDailyBudgetUsd: number;
  /** MRR = totalDailyBudgetUsd × 30 (a budget projection, undiscounted). */
  mrrUsd: number;
  /** ARR = totalDailyBudgetUsd × 365 (a budget projection, undiscounted). */
  arrUsd: number;
  activeCount: number;
  pausedCount: number;
...
export async function buildAccountsAudit(
  ...
  const rows: AccountRow[] = await Promise.all(
    [...pairs.values()].map(async (p): Promise<AccountRow> => {
      ...
      const { balance } = info;
      // The daily budget is the RAW configured ceiling — the usage discount is a charge modifier, never
      // applied to a config value. The ACTIVE verdict gates on the same raw budget vs the actual balance.
      return {
        orgId: p.orgId,
  ...
  // 4. Fleet stats — sum RAW daily budget over ACTIVE rows only (paused/inactive don't spend); MRR ×30,
  //    ARR ×365. Undiscounted budget projection (a budget is a config ceiling, not a charge). Active ⇒
  //    budget > 0 by the verdict rule, so the sum is over positive numbers.
  let totalDailyBudgetUsd = 0;
  let activeCount = 0;
  let pausedCount = 0;
  for (const row of rows) {
    if (row.status === "active") {
      totalDailyBudgetUsd += row.dailyBudgetUsd as number; // active ⇒ budget is a positive number
      activeCount += 1;
  ...
  // Round the fleet total to cents defensively (per-row budgets are already dollars-and-cents).
